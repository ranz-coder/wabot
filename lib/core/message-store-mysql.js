...
class MessageStore {
    ...
    addMessage(jid, msg) {
        ...
        if (msgId && this.pool) {
            this.pool.execute('INSERT INTO messages (jid, id, data, created_at) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE data = VALUES(data), created_at = VALUES(created_at)', [jid, msgId, JSON.stringify(msg), Date.now()]).then(() => {
                return this.pool.execute('DELETE FROM messages WHERE jid = ? AND id NOT IN (SELECT id FROM (SELECT id FROM messages WHERE jid = ? ORDER BY created_at DESC LIMIT ?) as tmp)', [jid, jid, this.max]);
            }).catch((error) => {
