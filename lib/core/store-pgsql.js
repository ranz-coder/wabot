...
class Store {
    ...
    async addMessage(jid, msg) {
        ...
        if (this.pool) {
            ...
            const current = previous
                .then(async () => {
                try {
                    await this.pool.query('INSERT INTO messages (jid, id, data, created_at) VALUES ($1, $2, $3, $4) ON CONFLICT (jid, id) DO UPDATE SET data = EXCLUDED.data, created_at = EXCLUDED.created_at', [jid, msgId, JSON.stringify(cleanedMsg), Date.now()]);
    ...
    async addStory(jid, story) {
        ...
        if (this.pool) {
            try {
                await this.pool.query('INSERT INTO stories (jid, id, data, created_at) VALUES ($1, $2, $3, $4) ON CONFLICT (jid, id) DO UPDATE SET data = EXCLUDED.data, created_at = EXCLUDED.created_at', [jid, storyId, JSON.stringify(this.toPOJO(story)), Date.now()]);
            }
            catch { }
            return;
        }
